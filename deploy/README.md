...
Весь стек поднимается одной командой через `docker compose`: PostgreSQL, API,
worker, IMAP-поллер, monitor и Caddy (раздаёт фронт + reverse-proxy +
авто-HTTPS). Файлы документов хранятся на диске (filesystem storage) в томе
`objects-data` — отдельное объектное хранилище не нужно.
...
Пересобираются только API/worker/imap/web. **PostgreSQL не трогается — данные
(БД в томе `postgres-data`, файлы в `objects-data`) сохраняются.** Изменения
схемы применяются миграциями автоматически и данные не удаляют.

...
PD is only for a slip that documents CASH leaving or entering a till. Two printed facts outrank the till-slip look and make it FP instead:
- the slip identifies the BUYER by IČO, DIČ or IČ DPH, or carries a number labelled "Faktúra" — then it is a zjednodušená faktúra (a real supplier invoice in receipt form, typical for METRO, fuel cards and wholesalers), not an anonymous cash purchase;
- the printed payment method is a card or a transfer ("Platba kartou", a terminal slip with AID/terminal number, "Prevodom", "Uhradené z účtu") — no cash moved, so it is not a cash document.
An anonymous slip with no buyer identifiers, paid in cash ("Hotovosť", "V hotovosti"), stays PD.

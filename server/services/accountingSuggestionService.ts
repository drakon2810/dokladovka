...
CUTTING ONE ITEM IN TWO. Sometimes the firm does not move a whole item to another account but splits the item itself — the second line does not exist on the invoice, the accountant creates it. The firm's own rows show it: an item whose text names a part of the original supply — a percentage, or a word for the deductible and the non-deductible half — posted to a different predkontácia is the tail of exactly such a cut. To propose one, return several "riadky" entries with the SAME index, each carrying "podiel" — the fraction of that item it takes. The fractions must add up to 1, and there must be at least two of them; anything else is dropped whole, because a partial cut would lose money from the document.
...
A cut is NEVER written on the invoice. The supplier bills one line of fuel; the accountant is the one who divides it. So do not wait for the document to name a non-deductible part — it never does, and its absence is not evidence against the cut. The evidence is the firm's own history: rows in "dennik" or "priklady" whose item text names a part of the original supply, posted to a different predkontácia, ARE the record of this cut. When such rows exist for THIS counterparty and the document has the matching kind of item, cut it the same way.
"rozuctovanie" is the same thing with the numbers: the items of this counterparty's last split documents, each with "suma" (the base) and "sumaDph" (its VAT), exactly as the accountant entered them. Compute "podiel" from the sums of the parts and "podielDph" from their VAT, each on its own — one does not follow from the other and in practice they differ, because a deduction can be capped by law while the cost is divided by use. Only the item of the SAME kind gets cut: when the history splits one kind of supply and leaves a neighbouring one whole, cut only the matching line and leave the rest, discounts included, alone.

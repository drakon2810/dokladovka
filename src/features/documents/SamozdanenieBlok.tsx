// Samozdanenie na prijatej faktúre bez DPH — blok nad zaúčtovaním. Počíta
// server (základ, daň, dátum, kódy z profilu klienta); editor len volí a
...
export function SamozdanenieBlok({ documentId, version, readOnly }: { documentId: string; version: number; readOnly: boolean }) {
  ...
  return (
    <div className="dk-card sz-blok" data-testid="samozdanenie">
      <div className="sz-head">
        <span className="dk-card-title">{t('samozdanenie.titul')}</span>
            ...
            {hodnota.volba === volba && volba === 'vytvorit' && (
              <div className="sz-obsah">
                ...
                <div className="sz-riadok">
                  ...
                  {upravitelny && (
                    <button type="button" className="sz-odkaz" onClick={() => setZmenaDruhu(!zmenaDruhu)}>{t('samozdanenie.zmenitDruh')}</button>
                  )}
                  {upravitelny && zmenaDruhu && (
                    <select
